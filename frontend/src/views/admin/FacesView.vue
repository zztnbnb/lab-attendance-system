...
<template>
  <PageHeader title="人脸档案" description="监督录入可直接激活；自助录入必须由管理员现场实时复验。"><el-button type="primary" :icon="Plus" @click="openEnroll">监督录入</el-button></PageHeader>
  ...
  <el-dialog v-model="captureDialog" :title="captureMode === 'enroll' ? '管理员监督录入' : `现场复验 · ${selectedProfile?.user_name}`" width="min(720px, 94vw)" :close-on-click-modal="false" @closed="stop">
    ...
    <div class="camera-stage"><video ref="video" muted playsinline /><div class="camera-guide" /><div class="camera-tip">正对镜头，并按提示缓慢转头</div></div>
