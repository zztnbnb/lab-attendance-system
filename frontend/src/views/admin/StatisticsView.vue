...
<template>
  ...
  <div v-loading="loading">
    ...
    <section class="panel ranking"><div class="panel__header"><h2>用户时长排名</h2><span>当前统计区间</span></div><el-table :data="stats?.ranking"><el-table-column type="index" label="排名" width="80" /><el-table-column prop="real_name" label="姓名" /><el-table-column prop="username" label="账号" /><el-table-column label="累计有效时长"><template #default="{ row }"><b class="accent">{{ formatDuration(row.duration_seconds) }}</b></template></el-table-column><el-table-column label="折合小时"><template #default="{ row }">{{ (row.duration_seconds / 3600).toFixed(2) }} h</template></el-table-column></el-table></section>
  </div>
</template>
